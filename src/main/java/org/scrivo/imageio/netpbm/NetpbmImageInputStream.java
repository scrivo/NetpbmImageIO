...
/**
 * NetpbmImageInputStream is a buffered (Image)InputStream that supports binary
 * access to the image stream for reading the binary content data and text
 * access for reading numbers and characters from the image stream.
 * 
 ...
 * 
 * Note: an earlier implementation used ImageInputStreamImpl.readLine to parse
 * character data from the stream, for whatever reason this proved to be
 * unacceptably slow.
 */
class NetpbmImageInputStream extends ImageInputStreamImpl {
	...
	/**
	 * Upper bound (exclusive) of the writeable part of the buffer (=BUF_SIZE or
	 * smaller if less than BUF_SIZE bytes were read from the stream).
	 */
	...
	/**
	 * Test if a byte from the stream is a white space character or a comment
	 * character.
	 * 
	 * @param c The character to test.
	 * @return True of given character is a white space/comment character.
	 */
	 ...
	 * 
	 * @param c The character to test.
	 * @return True of given character is a line break character.
	 */
	 ...
	 * 
	 * @param is The image input stream to use.
	 */
	...
	/**
	 * Gets the next string token form the stream. A string token is just a sequence
	 * of characters that does not contain white space (or a comment), i.e. the
	 * string '0010 1111' holds 2 tokens.
	 * 
	 * @return The next string token found in the stream (never an empty string), or
	 *         null if we are at the end of the file.
	 * @throws IOException
	 */
	...
	/**
	 * Read a single character token from the stream. This method differs from
	 * readStringToken in that the tokens do not necessarily need a whitespace
	 * seperator, i.e. the string '0010 1111' holds 8 tokens.
	 * 
	 * @return The next character token from the stream, or EOF if end of file.
	 * @throws IOException
	 */
	...
	/**
	 * Skip white space and comments in the stream. If the method exits normally
	 * (returns 0) then tokenChr will hold the first byte after the white space.
	 * 
	 * @return Return EOF (-1) if we reached the end of the stream, 0 if not.
	 * @throws IOException
	 */
