...
/**
 * The PbmImageReaderSpi class provides information about the plug-in, 
 * including the vendor name, plug-in version string and description, format 
 * name, file suffixes associated with the format, MIME types associated with 
 * the format, input source classes that the plug-in can handle, and the 
 * ImageWriterSpis of plug-ins that are able to interoperate specially with 
 * the reader. It also must provide an implementation of the canDecodeInput 
 * method, which is used to locate plug-ins based on the contents of a source 
 * image file.
 * 
 * @See
 * 		https://docs.oracle.com/javase/8/docs/technotes/guides/imageio/spec/extending.fm1.html
 */
public class PbmImageReaderSpi extends ImageReaderSpi {
	...
	private static final String DESCRIPTION = 
			"ImageIO plugin for Netpbm (pbm, pgm, ppm) files.";
	 ...
	 * first, whether the input parameter is an instance of a class that 
	 * the plug-in can understand, and second, whether the file contents 
	 * appear to be in the format handled by the plug-in. It must leave its 
	 * input in the same state as it was when it was passed in.
	 */
	@Override
